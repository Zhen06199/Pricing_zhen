...
def extract_all_runtime_data(log_text):
    ...
    for header, k_value, content in blocks:
        Providers = [int(c) for c in re.findall(r"TOTAL_NUM_PROVIDER:\s*(\d+)", content)]
        runtimes = [float(r) for r in re.findall(r"Runtime:\s*([\d.]+)", content)]
        k = int(k_value)
        if Providers and runtimes:
            data_by_k[k] = (Providers, runtimes)
    return data_by_k
...
for ax, k in zip(axes, selected_ks):
    Providers, runtimes = data_dict[k]
    ax.plot(Providers, runtimes, marker='o', color='blue', linewidth=2, markersize=6, label=f"K = {k}")
    ax.set_title(f"Runtime vs Providers (K={k})", fontweight='bold')
    ax.set_xlabel("Number of Providers", fontweight='bold')
...
print("✅ Saved as 'runtime_providers.png'")

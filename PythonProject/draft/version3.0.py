...
def probability(U_0, U_1, demand, P0, P1,budget, index):
    ...
    p_1 = np.exp(U_1 - index*demand * P1) if P0<=budget else 0
    if p_1+p_0==0:
        pr_0=0
        pr_1=0
    else:
...
for iteration in range(max_iterations):
    ...
    for c in range(customer):
        ...
        for P0 in P0_range:
            ...
            for P1 in P1_range:
                choice_0 = probability(Utility_0[c], Utility_1[c], demand[c], P0, P1,budget[c],sd_index)
    ...
    P0_range_new = np.arange(new_P0_min, new_P0_max , step_size)

    step_size = max((new_P1_max - new_P1_min) / 8, tolerance)  # 防止步长过小
    P1_range_new = np.arange(new_P1_min, new_P1_max , step_size)
